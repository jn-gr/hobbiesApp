...
const fetchUsers = async () => {
  try {
    ...
    const response = await fetch(`https://group39-web-apps-ec21653.apps.a.comp-teach.qmul.ac.uk/api/similar_users/?${params}`, {
      credentials: 'include',
      headers: {
...
const sendFriendRequest = async (userId: number) => {
  try {
    const csrfToken = await authStore.setCsrfToken()
    const response = await fetch(`https://group39-web-apps-ec21653.apps.a.comp-teach.qmul.ac.uk/api/friend_requests/send/${userId}/`, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'X-CSRFToken': csrfToken
