<script setup lang="ts">
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `https://group39-web-apps-ec22572.apps.a.comp-teach.qmul.ac.uk/api/similar_users/?${params}`,
        {
          credentials: 'include',
          headers: {
  ...
  const sendFriendRequest = async (userId: number) => {
    try {
      const csrfToken = await authStore.setCsrfToken()
      const response = await fetch(
        `https://group39-web-apps-ec22572.apps.a.comp-teach.qmul.ac.uk/api/friend_requests/send/${userId}/`,
        {
          method: 'POST',
          credentials: 'include',
          headers: {
...
<template>
  <div class="container mx-auto p-8">
    <div
      class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 space-y-6 md:space-y-0"
    >
      ...
      <div class="flex items-end gap-4 flex-wrap">
        <div class="space-y-2">
          ...
          <div class="flex items-center gap-2">
            <Input v-model="ageMin" type="number" placeholder="Min" class="w-24" />
            <span>-</span>
            <Input v-model="ageMax" type="number" placeholder="Max" class="w-24" />
          </div>
        </div>
    ...
    <div v-else-if="!isLoading && users.length === 0" class="text-center py-12">
      ...
      <p class="text-muted-foreground">
        Try adjusting your filters or add some hobbies to your profile to find users with similar
        interests.
      </p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <Card v-for="user in users" :key="user.id" class="relative" id="filteredUsers">
        ...
        <Button v-else class="absolute top-4 right-4" variant="secondary" size="sm" disabled>
          Friends
        </Button>

        <CardContent class="pt-6">
          <div class="flex items-start gap-4">
            <img :src="getAvatarUrl(user.name)" :alt="user.name" class="rounded-full w-16 h-16" />
            <div>
              ...
              <p class="text-sm text-muted-foreground">
                {{ user.common_hobbies }} shared
                {{ user.common_hobbies === 1 ? 'hobby' : 'hobbies' }}
              </p>
            </div>
          </div>
        </CardContent>
